...
from route_registry import load_routes, og_image, public_pages, site_url  # noqa: E402

SITE = site_url()
OG_IMAGE = og_image()
ROUTES = {route.file: route for route in load_routes()}
...
def breadcrumb_json(route) -> dict:
    items = [{"@type": "ListItem", "position": 1, "name": "Home", "item": f"{SITE}/"}]
    for idx, name in enumerate(route.breadcrumbs, start=2):
        if name == "World Chain":
            path = "/world-chain/"
        elif name == "World ID":
            path = "/world-id/"
            ...
            path = route.route
        items.append({"@type": "ListItem", "position": idx, "name": name, "item": f"{SITE}{path}"})
    return {"@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": items}


def build_jsonld_block(route) -> str:
    url = f"{SITE}{route.route}"
    org = {
    ...
    if route.application:
        blocks.append(json_script({
            ...
            "url": url,
            "applicationCategory": route.application["category"],
            "operatingSystem": "Web",
    ...
    if route.breadcrumbs:
        blocks.append(json_script(breadcrumb_json(route)))
    return "\n".join(blocks)
...
def upsert_static_seo(text: str, page: str) -> str:
    ...
    seo_block = build_seo_block(route.route, route.title, route.description)
    jsonld_block = build_jsonld_block(route)
    if jsonld_block:
        seo_block = seo_block + "\n" + jsonld_block
