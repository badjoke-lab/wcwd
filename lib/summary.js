...
/**
 * ★ Phase 0 最終判定
 * ok: 致命的エラーなし（=errorsが空）なら true
 * status:
 *   - "error"  : errors>0
 *   - "partial": errors==0 && warnings>0
 *   - "ok"     : errors==0 && warnings==0
 * degraded: warnings>0
 *
 * ※ UIは status を見るのが正しい（okだけで判断しない）
 */
export function finalizeStatus(summary) {
  const e = Array.isArray(summary?.errors) ? summary.errors : [];
  const w = Array.isArray(summary?.warnings) ? summary.warnings : [];

  const hasErrors = e.length > 0;

  summary.degraded = w.length > 0;
  summary.status = hasErrors ? "error" : (summary.degraded ? "partial" : "ok");
...
async function getRpcSection(rpcUrl, warnings, errors) {
  ...
  if (!feeHistoryRes.ok) pushWarn(warnings, "rpc", "eth_feeHistory", "failed_optional");
...
async function getCoinGeckoSection({ cgKey, warnings }) {
  // demo/pro の両対応。環境変数名は CG_KEY のままでOK。
  // CoinGecko側のプラン/キー種別で header 名が違う可能性があるので、両方投げる（害は少ない）
  const headers = {
    accept: "application/json",
    ...(cgKey ? { "x-cg-demo-api-key": cgKey, "x-cg-pro-api-key": cgKey } : {}),
  };
  ...
  // 401 は「キー権限/プラン制限」の可能性が高い＝致命傷ではないので warnings に残して partial 扱い
  if (!chartOk) {
    const reason =
      chartRes.status === 401 ? "unauthorized_or_plan_limit (status=401)" : `failed (status=${chartRes.status})`;
    pushWarn(warnings, "coingecko", "market_chart", reason);
  }
  ...
  return {
    ok: !!simple,
    mode: cgKey ? "key" : "public",
    coin_id: "worldcoin-wld",
    simple,
    chart7d_usd,
    note: `CoinGecko ${simple ? "ok" : "failed"}. mode=${cgKey ? "key" : "public"}. coin_id=worldcoin-wld. simple=${!!simple} markets=${chartOk}`,
    http_status: {
